...
func TestProxyHostMasking(t *testing.T) {
	...
	proxy := NewReverseProxy(upstreamHost, "http", rep, false, fakeProxyAddr, true, 0, testLogger(), nil, nil, 0, nil, nil, false)
	ps := httptest.NewServer(proxy)
	defer ps.Close()
...
func TestSubdomainRootRelativeIntegration(t *testing.T) {
	...
	rep, _ := NewReplacer("", false)
	proxy := NewReverseProxy(rootHost, "http", rep, false, "localhost:9999", false, 0, testLogger(), nil, nil, 0, nil, nil, false)
...
func TestExtraHeadersSentUpstream(t *testing.T) {
	...
	proxy := NewReverseProxy(host, "http", rep, false, "", true, 0, testLogger(), extra, nil, 0, nil, nil, false)
	ps := httptest.NewServer(proxy)
	defer ps.Close()
...
func TestFollowAlsoProxyDomainRedirect(t *testing.T) {
	...
	rep, _ := NewReplacer("", false)
	proxy := NewReverseProxy(targetHost, "http", rep, false, "localhost:9999", true, 0, testLogger(), nil, nil, 0, []string{alsoHost}, nil, false)
	ps := httptest.NewServer(proxy)
	defer ps.Close()
...
func TestSDPathNoTrailingSlash(t *testing.T) {
	...
	proxy := NewReverseProxy("localhost", "http", rep, false, "localhost:9999", true, 0, testLogger(), nil, nil, 0, []string{upHost}, nil, false)
	ps := httptest.NewServer(proxy)
	defer ps.Close()
